...
contains
  ...
  subroutine begin_parallel_write(file_id, dset_path, nlocal, group_id, xfer_id, partition_, ntotal, local_offset, hdferr)
  ...
  subroutine write_partition(group_id, partition_, xfer_id, hdferr)
    integer(hid_t), intent(in) :: group_id, xfer_id
    integer(int64), intent(in) :: partition_(0:)
    integer, intent(out) :: hdferr

    call write_i64_partition_dataset(group_id, trim(PARTITION_DATASET_NAME), partition_, xfer_id, hdferr)
  ...
  subroutine h5fort_write_r64_1d(file_id, dset_path, array, hdferr)
    ...
    call begin_parallel_write(file_id, dset_path, nlocal, group_id, xfer_id, partition_, ntotal, local_offset, hdferr)
    if (hdferr /= 0) return
    ...
    if (hdferr == 0) call write_partition(group_id, partition_, xfer_id, hdferr)
