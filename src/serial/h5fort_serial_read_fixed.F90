...
contains
  ...
  subroutine h5fort_read_i64_1d_fixed(file_id, dset_path, array, hdferr)
    integer(hid_t),   intent(in)  :: file_id
    character(len=*), intent(in)  :: dset_path
    integer(int64),        intent(out) :: array(:)
    integer,          intent(out) :: hdferr
    integer(int64), allocatable :: tmp(:)
    call h5fort_read_i64_1d(file_id, dset_path, tmp, hdferr)
    if (hdferr /= 0) return
    if (any(shape(tmp) /= shape(array))) then
      write(error_unit,*) "[h5fort/mode/read_fixed] ERROR: shape mismatch: dataset=", shape(tmp), " array=", shape(array)
      hdferr = -1; return
  ...
  subroutine h5fort_read_i64_2d_fixed(file_id, dset_path, array, hdferr)
    integer(hid_t),   intent(in)  :: file_id
    character(len=*), intent(in)  :: dset_path
    integer(int64),        intent(out) :: array(:, :)
    integer,          intent(out) :: hdferr
    integer(int64), allocatable :: tmp(:, :)
    call h5fort_read_i64_2d(file_id, dset_path, tmp, hdferr)
    if (hdferr /= 0) return
    if (any(shape(tmp) /= shape(array))) then
      write(error_unit,*) "[h5fort/mode/read_fixed] ERROR: shape mismatch: dataset=", shape(tmp), " array=", shape(array)
      hdferr = -1; return
  ...
  subroutine h5fort_read_i64_3d_fixed(file_id, dset_path, array, hdferr)
    integer(hid_t),   intent(in)  :: file_id
    character(len=*), intent(in)  :: dset_path
    integer(int64),        intent(out) :: array(:, :, :)
    integer,          intent(out) :: hdferr
    integer(int64), allocatable :: tmp(:, :, :)
    call h5fort_read_i64_3d(file_id, dset_path, tmp, hdferr)
    if (hdferr /= 0) return
    if (any(shape(tmp) /= shape(array))) then
      write(error_unit,*) "[h5fort/mode/read_fixed] ERROR: shape mismatch: dataset=", shape(tmp), " array=", shape(array)
      hdferr = -1; return
  ...
  subroutine h5fort_read_i64_4d_fixed(file_id, dset_path, array, hdferr)
    integer(hid_t),   intent(in)  :: file_id
    character(len=*), intent(in)  :: dset_path
    integer(int64),        intent(out) :: array(:, :, :, :)
    integer,          intent(out) :: hdferr
    integer(int64), allocatable :: tmp(:, :, :, :)
    call h5fort_read_i64_4d(file_id, dset_path, tmp, hdferr)
    if (hdferr /= 0) return
    if (any(shape(tmp) /= shape(array))) then
      write(error_unit,*) "[h5fort/mode/read_fixed] ERROR: shape mismatch: dataset=", shape(tmp), " array=", shape(array)
      hdferr = -1; return
  ...
  subroutine h5fort_read_lgc_1d_fixed(file_id, dset_path, array, hdferr)
    integer(hid_t),   intent(in)  :: file_id
    character(len=*), intent(in)  :: dset_path
    logical,          intent(out) :: array(:)
    integer,          intent(out) :: hdferr
    logical, allocatable :: tmp(:)
    call h5fort_read_lgc_1d(file_id, dset_path, tmp, hdferr)
    if (hdferr /= 0) return
    if (any(shape(tmp) /= shape(array))) then
      write(error_unit,*) "[h5fort/mode/read_fixed] ERROR: shape mismatch: dataset=", shape(tmp), " array=", shape(array)
      hdferr = -1; return
  ...
  subroutine h5fort_read_lgc_2d_fixed(file_id, dset_path, array, hdferr)
    integer(hid_t),   intent(in)  :: file_id
    character(len=*), intent(in)  :: dset_path
    logical,          intent(out) :: array(:, :)
    integer,          intent(out) :: hdferr
    logical, allocatable :: tmp(:, :)
    call h5fort_read_lgc_2d(file_id, dset_path, tmp, hdferr)
    if (hdferr /= 0) return
    if (any(shape(tmp) /= shape(array))) then
      write(error_unit,*) "[h5fort/mode/read_fixed] ERROR: shape mismatch: dataset=", shape(tmp), " array=", shape(array)
      hdferr = -1; return
  ...
  subroutine h5fort_read_lgc_3d_fixed(file_id, dset_path, array, hdferr)
    integer(hid_t),   intent(in)  :: file_id
    character(len=*), intent(in)  :: dset_path
    logical,          intent(out) :: array(:, :, :)
    integer,          intent(out) :: hdferr
    logical, allocatable :: tmp(:, :, :)
    call h5fort_read_lgc_3d(file_id, dset_path, tmp, hdferr)
    if (hdferr /= 0) return
    if (any(shape(tmp) /= shape(array))) then
      write(error_unit,*) "[h5fort/mode/read_fixed] ERROR: shape mismatch: dataset=", shape(tmp), " array=", shape(array)
      hdferr = -1; return
  ...
  subroutine h5fort_read_lgc_4d_fixed(file_id, dset_path, array, hdferr)
    integer(hid_t),   intent(in)  :: file_id
    character(len=*), intent(in)  :: dset_path
    logical,          intent(out) :: array(:, :, :, :)
    integer,          intent(out) :: hdferr
    logical, allocatable :: tmp(:, :, :, :)
    call h5fort_read_lgc_4d(file_id, dset_path, tmp, hdferr)
    if (hdferr /= 0) return
    if (any(shape(tmp) /= shape(array))) then
      write(error_unit,*) "[h5fort/mode/read_fixed] ERROR: shape mismatch: dataset=", shape(tmp), " array=", shape(array)
      hdferr = -1; return
